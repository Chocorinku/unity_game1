...
namespace SupanthaPaul {
    public class PlayerController_Oneda : MonoBehaviour {
        ...
        Vector2 endTouchPos;
        private Vector3 mag;
        public float flickGraceTimer = 0.5f;    //????????
        public float flickGraceLength = 10f;    //??????????
        ...
        public float coordinateAngle;    //?????????(????)
        public float circumferentialAngle;  //???180??????????????(????)
        /// </summary>
        ...
        private void Update()
        {
            ...
            if (Input.GetMouseButtonUp(0) == true)
            {
                ...
                coordinateAngle = Mathf.Atan2(mag.y, mag.x);         //????????????????????????????????
                Debug.Log(new Vector2(Mathf.Cos(coordinateAngle), Mathf.Sin(coordinateAngle)));

                circumferentialAngle = Mathf.Atan2(mag.y, mag.x) * Mathf.Rad2Deg;    //???180??????????????(????)
                Debug.Log("??? " + circumferentialAngle);
            ...
            if ((InputSystem.Jump() && m_extraJumps > 0 && !isGrounded && !m_wallGrabbing) ||
                    (Input.GetMouseButtonDown(0) && m_extraJumps > 0 && !isGrounded && !m_wallGrabbing))
            {   // extra jumping
                ...
                doubleTapFlag = false;
                  ...
                  FloatingJoystick.Vertical > 0.3f && jumpFlag && (isGrounded || m_groundedRemember > 0f))
            {  // normal single jumping
                jumpFlag = false;
                ...
                PoolManager.instance.ReuseObject(jumpEffect, groundCheck.position, Quaternion.identity);
                  ...
                  (FloatingJoystick.Vertical > 0.3f && m_wallGrabbing && moveInput != m_onWallSide))
            {      // wall jumping off the wall
        ...
        {
            return (Application.isEditor && InputSystem.Dash()) || (!Application.isEditor && flickFlag);
        }
